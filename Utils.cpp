...
int doAlignment(string pdb1, string pdb2, TMatrix &tmatrix, int alignment_opt=0) {
    clock_t t1, t2;
    t1 = clock();
    ...
    int    mm_opt= alignment_opt;         // do not perform MM-align
...
TMatrix getMatrix(string pdb_str1, string pdb_str2, int alignment_opt=0)
{
    TMatrix tmatrix;
    doAlignment(pdb_str1, pdb_str2, tmatrix, alignment_opt);   // does TMalign by default
    return tmatrix;
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2) {
        p1 = argv[1];
        p2 = argv[2];
        tmatrix = getMatrix(p1, p2);

    } else {
        // run one of the tests
        tmatrix = getMatrix(FOUR_RMG_BUILTIN, J3_SIRT1);     // pdb strings
        printf(("Enter 2 proteins\n"));
    }

    tmatrix.print_transform();
    tmatrix.print_rotation();

    // other tests:
    // tmatrix = getMatrix("PDB1.pdb", "PDB2.pdb");
    // tmatrix = getMatrix(FOUR_RMG_BUILTIN, "PDB1.pdb");
